...
from dao.analytics import SingleGroupAnalytics

from backend_db_lib.models import Group, LPAAudit, LPAAnswer
...
def get_analytics_by_group(authorization: Union[str, None] = Header(default=None)) -> List[SingleGroupAnalytics]:
    ...
    with dbm.create_session() as session:
        ...
        groups = session.query(Group).filter(Group.company_id == company_id).all()


        response = []
        for group in groups:
            # Get all Audits of this group
            audits = session.query(LPAAudit).filter(LPAAudit.assigned_group_id == group.id).all()
